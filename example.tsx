...
export function reorder<Value>({
  ...
  const start = Array.isArray(startIndex) ? startIndex : [startIndex]
  if (start.some(i =>  i === -1) || finishIndex === -1) {
    return list;
  }

  let result = Array.from(list);
  const removed = start.map(i => result.slice(i, i+1)).flat();
  result = result.filter(i => !removed.includes(i))
...
function ListItem({
  ...
  const { registerItem, instanceId, setSelected, selected } = useListContext();
  ...
  }, [instanceId, item, index, registerItem, selected]);

  return (
    <Fragment>
      <Box ref={ref} xcss={[
        listItemContainerStyles,
        selected.includes(item.id) && xcss({ border: '1px solid red'}),
]} onClick={(e) => {
          e.persist()
  ...
  const reorderItem = useCallback(
    ({
      ...
      setListState(listState => {
        const item = listState.items[startIndex[0]];
console.log('SET LAST', { item, startIndex, items: listState.items })
        return {
          items: reorder({
  ...
  useEffect(() => {
    return monitorForElements({
      ...
      onDrop({ location, source }) {
        ...
        const startIndex = selected.map(s => items.findIndex(i => i.id === s));
  ...
  }, [instanceId, items, reorderItem, registry, selected]);
  ...
  }, [registry.register, reorderItem, instanceId, getListLength, selected, setSelected]);
